...
class Backgammon(StochasticGame):
    ...
    def result(self, state, move):
        ...
        return StochasticGameState(to_move=to_move, utility=self.compute_utility(board, player),
                                   board=board, moves=moves, chance=None)

    def chances(self, state):
        """Return a list of dice rolls."""
        return list(itertools.product([1, 2, 3, 4, 5, 6], repeat=2))

    def outcome(self, state, chance):
        """Return a state with a chance (dice roll) played."""
        return StochasticGameState(to_move=state.to_move, utility=state.utility,
                                   board=state.board, moves=state.moves, chance=chance)

    def probability(self, chance):
        """Return the probability of a particular chance (dice roll)."""
        return 1.0 / 36.0
    ...
    def get_all_moves(self, board, player):
        ...
        for i in range(24):
            if board[i][player] > 0:
                if self.direction[player] == -1:
                    for j in range(1, min(board[i][player], 6) + 1):
                        moves.append([i, i - j])
                else:
                    for j in range(1, min(board[i][player], 6) + 1):
                        moves.append([i, i + j])
    ...
    def is_legal_move(self, board, move, chance, player):
        ...
        for i, position in enumerate(move):
            ...
            if board[position][other] > 1:
                return False
        return True

    def move_checker(self, board, src, die, player):
        """Move a checker on the board."""
        board[src][player] -= 1
        dst = src + (die * self.direction[player])
        if dst < 0 or dst > 23:
    ...
    def compute_utility(self, board, player):
        ...
        if all(count == 0 for count in board[i].values() for i in range(24)):
